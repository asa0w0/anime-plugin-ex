...
class AnimescheduleService
  ...
  def self.fetch_anime_details(route)
    api_key = SiteSetting.anime_animeschedule_api_key
    return nil if api_key.blank?
    ...
    if response.code == '200'
      ...
      Rails.logger.warn("[AnimeSchedule] Details API returned #{response.code} for #{route}")
      nil
    end
  rescue => e
    Rails.logger.error("[AnimeSchedule] Error fetching anime details for #{route}: #{e.message}")
    nil
  end
end
